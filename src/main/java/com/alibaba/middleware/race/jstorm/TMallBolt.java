...
public class TMallBolt implements IRichBolt {
    ...
    private synchronized void handleOldMessages() {
        ...
        for (int i = 0; i < metaMessages.size(); i++) {
            ...
            if (tmallMap.get(metaMessage.getOrderId()) != null && tmallMap.get(metaMessage.getOrderId()).equals(RaceConfig.MqTmallTradeTopic)) {
    ...
    public void execute(Tuple tuple) {
        /*System.out.println("==============TMallBolt bolt begin==========");
        ...
        if (metaTuple != null ) {
                if (RaceConfig.MqTmallTradeTopic.equals(metaTuple.getTopic())) {
                    ...
                    tmallMap.put(metaTuple.getOrderId(), RaceConfig.MqTaobaoTradeTopic);
                } else if (tmallMap.get(metaTuple.getOrderId()) == null) {
                    metaMessages.add(metaTuple);
                } else if (RaceConfig.MqPayTopic.equals(metaTuple.getTopic())) {
                    if (tmallMap.get(metaTuple.getOrderId()).equals(RaceConfig.MqTmallTradeTopic)) {
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declare(new Fields("time", "tmMeta"));
    }
